...
var F = (t, A, e) => A in t ? v(t, A, { enumerable: !0, configurable: !0, writable: !0, value: e }) : t[A] = e;
var i = (t, A, e) => F(t, typeof A != "symbol" ? A + "" : A, e);
import n from "p5";
...
class y {
  ...
  _getMaxGlyphDimensions(A) {
    return this._characterGlyphs.reduce(
      (e, r) => {
        const s = r.getPath(0, 0, A).getBoundingBox();
        return {
  ...
  drawCharacters(A) {
    ...
    for (let e = 0; e < this._characterGlyphs.length; e++) {
      const r = e % this._charsetCols, s = Math.floor(e / this._charsetCols), o = this._maxGlyphDimensions.width * r - this._maxGlyphDimensions.width * this._charsetCols / 2, a = this._maxGlyphDimensions.height * s - this._maxGlyphDimensions.height * this._charsetRows / 2;
  ...
  getCharsetColorArray(A) {
    return (Array.isArray(A) ? A : Array.from(A)).map((r) => {
      const s = this._characterGlyphs.find(
        (o) => o.unicodes.includes(r.codePointAt(0))
      );
      if (!s)
        throw new g(`Could not find character in character set: ${r}`);
      return [s.r, s.g, s.b];
    });
  }
  /**
...
class C {
  constructor(A, e, r, s) {
    ...
    this.p = A, this.grid = e, this.characterSet = r, this._options = s, this._options.characterColor && (this._options.characterColor = this.p.color(this._options.characterColor)), this._options.backgroundColor && (this._options.backgroundColor = this.p.color(this._options.backgroundColor)), this._primaryColorSampleFramebuffer = this.p.createFramebuffer({
      ...
      depthFormat: this.p.UNSIGNED_INT,
      textureFiltering: this.p.NEAREST
    }), this._shader = this.p.createShader(h, M);
  }
  /**
   ...
   */
  updateOptions(A) {
    G(A), A != null && A.characterColor && (A.characterColor = this.p.color(A.characterColor)), A != null && A.backgroundColor && (A.backgroundColor = this.p.color(A.backgroundColor)), this._options = {
      ...this._options,
      ...A
...
class z extends C {
  constructor(e, r, s, o) {
    ...
    this.colorSampleShader = this.p.createShader(h, U), this.asciiCharacterShader = this.p.createShader(h, Y), this.colorSampleFramebuffer = this.p.createFramebuffer({
      density: 1,
      width: this.grid.cols,
      height: this.grid.rows,
      depthFormat: this.p.UNSIGNED_INT,
...
class R extends C {
  constructor(e, r, s, o) {
    ...
    this.characterSelectionShader = this.p.createShader(h, m(this.characterSet.asciiFontTextureAtlas.fontSize)), this.brightnessSampleShader = this.p.createShader(h, P(this.grid.cellHeight, this.grid.cellWidth)), this.colorSampleShader = this.p.createShader(h, I(16, this.grid.cellHeight, this.grid.cellWidth)), this.brightnessSplitShader = this.p.createShader(h, k), this.brightnessSampleFramebuffer = this.p.createFramebuffer({
      density: 1,
      width: this.grid.cols,
      height: this.grid.rows,
      depthFormat: this.p.UNSIGNED_INT,
  ...
  resetShaders() {
    this.characterSelectionShader = this.p.createShader(h, m(this.characterSet.asciiFontTextureAtlas.fontSize)), this.brightnessSampleShader = this.p.createShader(h, P(this.grid.cellHeight, this.grid.cellWidth)), this.colorSampleShader = this.p.createShader(h, I(16, this.grid.cellHeight, this.grid.cellWidth));
  }
  render(e, r) {
...
class j extends C {
  constructor(e, r, s, o) {
    ...
    this.sobelShader = this.p.createShader(h, X), this.sampleShader = this.p.createShader(h, p(16, this.grid.cellHeight, this.grid.cellWidth)), this.colorSampleShader = this.p.createShader(h, N), this.asciiCharacterShader = this.p.createShader(h, O), this.sobelFramebuffer = this.p.createFramebuffer({
      depthFormat: this.p.UNSIGNED_INT,
      textureFiltering: this.p.NEAREST
  ...
  resetShaders() {
    this.sampleShader = this.p.createShader(h, p(16, this.grid.cellHeight, this.grid.cellWidth));
  }
  updateOptions(e) {
...
var K = "precision mediump float;uniform sampler2D u_image;varying vec2 v_texCoord;void main(){vec4 color=texture2D(u_image,v_texCoord);float luminance=0.299*color.r+0.587*color.g+0.114*color.b;color.rgb=vec3(luminance);gl_FragColor=color;}", W = "precision mediump float;uniform sampler2D u_sketchTexture;uniform sampler2D u_previousColorTexture;uniform sampler2D u_sampleTexture;uniform sampler2D u_sampleReferenceTexture;uniform vec2 u_gridCellDimensions;uniform int u_sampleMode;uniform vec4 u_staticColor;void main(){vec2 cellCoord=floor(gl_FragCoord.xy);vec2 cellSizeInTexCoords=vec2(1.0)/u_gridCellDimensions;vec2 cellCenterTexCoord=(cellCoord+vec2(0.5))*cellSizeInTexCoords;bool isMatchingSample=texture2D(u_sampleTexture,cellCenterTexCoord)==texture2D(u_sampleReferenceTexture,cellCenterTexCoord);if(isMatchingSample){gl_FragColor=texture2D(u_previousColorTexture,cellCenterTexCoord);return;}else if(u_sampleMode==0){gl_FragColor=texture2D(u_sketchTexture,cellCenterTexCoord);}else{gl_FragColor=u_staticColor;}}", V = "precision mediump float;uniform sampler2D u_prevAsciiCharacterTexture;uniform sampler2D u_prevGradientTexture;uniform sampler2D u_nextGradientTexture;uniform vec2 u_resolution;void main(){vec2 uv=gl_FragCoord.xy/u_resolution;vec4 prevAscii=texture2D(u_prevAsciiCharacterTexture,uv);vec4 prevGradient=texture2D(u_prevGradientTexture,uv);vec4 nextGradient=texture2D(u_nextGradientTexture,uv);bool colorsMatch=prevGradient==nextGradient;gl_FragColor=colorsMatch ? prevAscii : nextGradient;}";
class L extends C {
  constructor(e, r, s, o, a) {
    ...
    this.gradientManager = o, this.grayscaleShader = this.p.createShader(h, K), this.colorSampleShader = this.p.createShader(h, W), this.asciiCharacterShader = this.p.createShader(h, V), this.grayscaleFramebuffer = this.p.createFramebuffer({
      density: 1,
      width: this.grid.cols,
      height: this.grid.rows,
      depthFormat: this.p.UNSIGNED_INT,
...
class b {
  ...
  updateFramebuffer() {
    ...
    for (let r = 0; r < A; r++) {
      const s = r < this._colors.length ? this.p.color(this._colors[r]) : this.p.color(0, 0, 0, 0), o = 4 * r;
      this.framebuffer.pixels[o] = this.p.red(s), this.framebuffer.pixels[o + 1] = this.p.green(s), this.framebuffer.pixels[o + 2] = this.p.blue(s), this.framebuffer.pixels[o + 3] = this.p.alpha(s);
    }
...
const f = (t, A, e, r) => {
  if (typeof t != "number" || t < A || t > e)
    throw new g(
      `Invalid ${r} value '${t}'. Expected a number between ${A} and ${e}.`
    );
}, Z = (t, A) => {
  const [e, r] = [t, A].map((s) => s.split(".").map(Number));
  for (let s = 0; s < Math.max(e.length, r.length); s++) {
    const o = e[s] ?? 0, a = r[s] ?? 0;
    if (o !== a) return o > a ? 1 : -1;
  }
  return 0;
};
class d {
  constructor(A, e, r, s, o, a, B) {
    i(this, "_brightnessStart");
    i(this, "_brightnessEnd");
    i(this, "enabled");
    i(this, "_onPaletteChangeCallback");
    i(this, "_palette");
    this.p = A, this._fontTextureAtlas = e, this._shader = r, this._characters = B, this._palette = new b(this.p, s), this._brightnessStart = Math.floor(o / 255 * 100) / 100, this._brightnessEnd = Math.ceil(a / 255 * 100) / 100, this.enabled = !0;
  }
  /**
  ...
  set brightnessStart(A) {
    f(A, 0, 255, "brightness start"), this._brightnessStart = A;
  }
  /**
   ...
   */
  set brightnessEnd(A) {
    f(A, 0, 255, "brightness start"), this._brightnessEnd = A;
  }
  /**
   ...
   */
  set characters(A) {
    this._characters = A, this.p._setupDone && this.palette.setColors(this._fontTextureAtlas.getCharsetColorArray(A));
  }
  // Getters
  get characters() {
    return this._characters;
...
class $ extends d {
  constructor(e, r, s, o, a, B, E, Q) {
    super(e, r, s, o, a, B, E);
    i(this, "direction");
    i(this, "angle");
    i(this, "speed");
    i(this, "zigzag");
    this.p = e, this._fontTextureAtlas = r, this._shader = s, this.direction = Q.direction, this.angle = Q.angle, this.speed = Q.speed, this.zigzag = Q.zigzag;
  }
  setUniforms(e, r) {
...
class q extends d {
  constructor(e, r, s, o, a, B, E, Q) {
    ...
    i(this, "speed");
    i(this, "density");
    this.p = e, this._fontTextureAtlas = r, this._shader = s, this.direction = Q.direction, this.centerX = Q.centerX, this.centerY = Q.centerY, this.speed = Q.speed, this.density = Q.density;
  }
  setUniforms(e, r) {
...
class AA extends d {
  constructor(e, r, s, o, a, B, E, Q) {
    ...
    i(this, "centerY");
    i(this, "radius");
    this.p = e, this._fontTextureAtlas = r, this._shader = s, this.direction = Q.direction, this.centerX = Q.centerX, this.centerY = Q.centerY, this.radius = Q.radius;
  }
  setUniforms(e, r) {
...
class eA extends d {
  constructor(e, r, s, o, a, B, E, Q) {
    ...
    i(this, "centerY");
    i(this, "speed");
    this.p = e, this._fontTextureAtlas = r, this._shader = s, this.centerX = Q.centerX, this.centerY = Q.centerY, this.speed = Q.speed;
  }
  setUniforms(e, r) {
...
class gA {
  constructor(A, e) {
    ...
    i(this, "_gradientConstructors", {
      linear: (A, e, r, s, o, a, B, E) => new $(A, e, r, s, o, a, B, E),
      spiral: (A, e, r, s, o, a, B, E) => new q(A, e, r, s, o, a, B, E),
      radial: (A, e, r, s, o, a, B, E) => new AA(A, e, r, s, o, a, B, E),
      conical: (A, e, r, s, o, a, B, E) => new eA(A, e, r, s, o, a, B, E),
      noise: (A, e, r, s, o, a, B, E) => new rA(A, e, r, s, o, a, B, E)
    });
    i(this, "_gradients", []);
    this.p = A, this.fontTextureAtlas = e;
    for (const r of Object.keys(this.gradientShaderSources)) {
      const s = this.gradientShaderSources[r];
      this.gradientShaders[r] = this.p.createShader(h, s);
    }
  }
  /**
...
class BA {
  constructor(A, e, r) {
    ...
    }, this.gradientManager = new gA(A, this.fontTextureAtlas), this.gradientCharacterSet = new c(
      this.p,
      r,
      u.characters
    ), this._renderers = [
      new z(this.p, this.grid, new c(this.p, r, u.characters), { ...u }),
      new R(this.p, this.grid, new c(this.p, r, _.characters), { ..._ }),
      new L(this.p, this.grid, this.gradientCharacterSet, this.gradientManager, { ...H }),
      new j(this.p, this.grid, new c(this.p, r, w.characters), { ...w }),
      new C(this.p, this.grid, new c(this.p, r, u.characters), { ...J })
...
class QA {
  constructor() {
    ...
    this.borderColor = "#000000", this._fontSize = 16, this.postDrawFunction = null;
  }
  /**
   ...
   */
  setup() {
    this.asciiFontTextureAtlas = new y(this.p, this._font, this._fontSize), this.grid = new T(
      this.p,
      this.asciiFontTextureAtlas.maxGlyphDimensions.width,
      this.asciiFontTextureAtlas.maxGlyphDimensions.height
    ), this.rendererManager = new BA(this.p, this.grid, this.asciiFontTextureAtlas), this.sketchFramebuffer = this.p.createFramebuffer({
      depthFormat: this.p.UNSIGNED_INT,
      textureFiltering: this.p.NEAREST
    });
  }
...
function hA(t) {
  n.prototype.setupAsciify = function() {
    ...
    this._incrementPreload(), this.loadAsciiFont(S), EA(this), t.setup(), this.setupAsciify();
  });
}
function nA(t) {
  n.prototype.loadAsciiFont = function(A) {
    const e = (r) => {
      t.font = r, this._decrementPreload();
    };
    if (typeof A == "string")
      this.loadFont(
        A,
        ...
        () => {
          throw new g(`loadAsciiFont() | Failed to load font from path: '${A}'`);
        }
      );
    else if (A instanceof n.Font)
      e(A);
    else
      throw new g("loadAsciiFont() | Invalid font parameter. Expected a path, base64 string, or p5.Font object.");
  }, n.prototype.registerMethod("init", function() {
    this._incrementPreload(), this.loadAsciiFont(S);
  });
}
function lA(t) {
...
function cA(t, A, e, r, s, o) {
  if (typeof A != "string")
    throw new g("Gradient name must be a string");
  if (!t.gradientConstructors[A])
    throw new g(
      `Gradient '${A}' does not exist! Available gradients: ${Object.keys(t.gradientConstructors).join(", ")}`
    );
  if (typeof e != "number")
    throw new g("Brightness start value must be a number");
  if (typeof r != "number")
    throw new g("Brightness end value must be a number");
  if (f(e, 0, 255, "brightness start"), f(r, 0, 255, "brightness end"), typeof s != "string")
    throw new g("Characters must be a string");
  if (s.length === 0)
    throw new g("Characters string cannot be empty");
  if (!o || typeof o != "object" || Array.isArray(o))
    throw new g("User parameters must be an object");
  const a = Object.keys(t.gradientParams[A]), B = Object.keys(o).filter((E) => !a.includes(E));
  if (B.length > 0)
    throw new g(
      `Invalid parameter(s) for gradient '${A}': ${B.join(", ")}
Valid parameters are: ${a.join(", ")}`
    );
}
function CA(t) {
  n.prototype.addAsciiGradient = function(A, e, r, s, o = {}) {
    return cA(
      t.rendererManager.gradientManager,
      A,
      e,
      r,
      s,
      o
    ), t.rendererManager.gradientManager.addGradient(
      A,
      e,
      r,
      s,
    ...
    t.rendererManager.gradientManager.removeGradient(A);
  };
}
function dA(t) {
  n.prototype.drawAsciify = function() {
  }, n.prototype.registerMethod("pre", function() {
    t.sketchFramebuffer.begin(), this.clear(), this.push();
  }), n.prototype.registerMethod("post", function() {
    this.pop(), t.sketchFramebuffer.end(), t.rendererManager.render(t.sketchFramebuffer, t.borderColor), this.drawAsciify();
  });
}
const l = new QA();
