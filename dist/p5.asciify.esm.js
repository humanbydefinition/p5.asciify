...
class l {
  constructor(A, e, r, i) {
    ...
    this.p = A, this.grid = e, this.fontTextureAtlas = r, this._options = i, this._options.characterColor && (this._options.characterColor = this.p.color(this._options.characterColor)), this._options.backgroundColor && (this._options.backgroundColor = this.p.color(this._options.backgroundColor)), this.characterColorPalette = new w(this.p, this.fontTextureAtlas.getCharsetColorArray(this._options.characters)), this._primaryColorSampleFramebuffer = this.p.createFramebuffer({
      ...
      depthFormat: this.p.UNSIGNED_INT,
      textureFiltering: this.p.NEAREST
    }), this._shader = this.p.createShader(Q, F);
  }
  /**
...
class u extends l {
  constructor(e, r, i, s) {
    ...
    this.colorSampleShader = this.p.createShader(Q, T), this.asciiCharacterShader = this.p.createShader(Q, y), this.colorSampleFramebuffer = this.p.createFramebuffer({
      density: 1,
      width: this.grid.cols,
      height: this.grid.rows,
      depthFormat: this.p.UNSIGNED_INT,
...
class P extends l {
  constructor(e, r, i, s) {
    ...
    this.characterSelectionShader = this.p.createShader(Q, D(this.fontTextureAtlas.fontSize)), this.brightnessSampleShader = this.p.createShader(Q, f(this.grid.cellHeight, this.grid.cellWidth)), this.colorSampleShader = this.p.createShader(Q, m(16, this.grid.cellHeight, this.grid.cellWidth)), this.brightnessSplitShader = this.p.createShader(Q, G), this.brightnessSampleFramebuffer = this.p.createFramebuffer({
      density: 1,
      width: this.grid.cols,
      height: this.grid.rows,
      depthFormat: this.p.UNSIGNED_INT,
...
class I extends l {
  constructor(e, r, i, s) {
    ...
    this.sobelShader = this.p.createShader(Q, k), this.sampleShader = this.p.createShader(Q, p(16, this.grid.cellHeight, this.grid.cellWidth)), this.colorSampleShader = this.p.createShader(Q, M), this.inversionShader = this.p.createShader(Q, U), this.asciiCharacterShader = this.p.createShader(Q, Y), console.log("characters", this._options.characters), this.sobelFramebuffer = this.p.createFramebuffer({
      depthFormat: this.p.UNSIGNED_INT,
      textureFiltering: this.p.NEAREST
...
var z = "precision mediump float;uniform sampler2D u_image;varying vec2 v_texCoord;void main(){vec4 color=texture2D(u_image,v_texCoord);float luminance=0.299*color.r+0.587*color.g+0.114*color.b;color.rgb=vec3(luminance);gl_FragColor=color;}", R = "precision mediump float;uniform sampler2D u_sketchTexture;uniform sampler2D u_previousColorTexture;uniform sampler2D u_sampleTexture;uniform sampler2D u_sampleReferenceTexture;uniform vec2 u_gridCellDimensions;uniform int u_sampleMode;uniform vec4 u_staticColor;void main(){vec2 cellCoord=floor(gl_FragCoord.xy);vec2 cellSizeInTexCoords=vec2(1.0)/u_gridCellDimensions;vec2 cellCenterTexCoord=(cellCoord+vec2(0.5))*cellSizeInTexCoords;bool isMatchingSample=texture2D(u_sampleTexture,cellCenterTexCoord)==texture2D(u_sampleReferenceTexture,cellCenterTexCoord);if(isMatchingSample){gl_FragColor=texture2D(u_previousColorTexture,cellCenterTexCoord);return;}else if(u_sampleMode==0){gl_FragColor=texture2D(u_sketchTexture,cellCenterTexCoord);}else{gl_FragColor=u_staticColor;}}", N = "precision mediump float;uniform sampler2D u_sampleTexture;uniform sampler2D u_sampleReferenceTexture;uniform sampler2D u_previousInversionTexture;uniform vec2 u_gridCellDimensions;uniform bool u_invert;void main(){vec2 cellCoord=floor(gl_FragCoord.xy);vec2 cellSizeInTexCoords=vec2(1.0)/u_gridCellDimensions;vec2 cellCenterTexCoord=(cellCoord+vec2(0.5))*cellSizeInTexCoords;bool isMatchingSample=texture2D(u_sampleTexture,cellCenterTexCoord)==texture2D(u_sampleReferenceTexture,cellCenterTexCoord);if(!isMatchingSample){if(u_invert){gl_FragColor=vec4(1.0);}else{gl_FragColor=vec4(vec3(0.0),1.0);}return;}else{gl_FragColor=texture2D(u_previousInversionTexture,cellCenterTexCoord);}}", X = "precision mediump float;uniform sampler2D u_prevAsciiCharacterTexture;uniform sampler2D u_prevGradientTexture;uniform sampler2D u_nextGradientTexture;uniform vec2 u_resolution;void main(){vec2 uv=gl_FragCoord.xy/u_resolution;vec4 prevAscii=texture2D(u_prevAsciiCharacterTexture,uv);vec4 prevGradient=texture2D(u_prevGradientTexture,uv);vec4 nextGradient=texture2D(u_nextGradientTexture,uv);bool colorsMatch=prevGradient==nextGradient;gl_FragColor=colorsMatch ? prevAscii : nextGradient;}";
class _ extends l {
  constructor(e, r, i, s, a) {
    ...
    this.gradientManager = s, this.grayscaleShader = this.p.createShader(Q, z), this.colorSampleShader = this.p.createShader(Q, R), this.inversionShader = this.p.createShader(Q, N), this.asciiCharacterShader = this.p.createShader(Q, X), this.grayscaleFramebuffer = this.p.createFramebuffer({
      density: 1,
      width: this.grid.cols,
      height: this.grid.rows,
      depthFormat: this.p.UNSIGNED_INT,
...
class C {
  constructor(A, e, r, i, s, a) {
    ...
    t(this, "_palette");
    this.p = A, this._fontTextureAtlas = e, this._shader = r, this._palette = new w(this.p, this._fontTextureAtlas.getCharsetColorArray(a)), this._brightnessStart = Math.floor(i / 255 * 100) / 100, this._brightnessEnd = Math.ceil(s / 255 * 100) / 100, this.enabled = !0;
  }
  /**
...
class sA {
  constructor(A, e) {
    ...
    t(this, "_gradientConstructors", {
      linear: (A, e, r, i, s, a, E) => new V(A, e, r, i, s, a, E),
      spiral: (A, e, r, i, s, a, E) => new $(A, e, r, i, s, a, E),
      radial: (A, e, r, i, s, a, E) => new q(A, e, r, i, s, a, E),
      conical: (A, e, r, i, s, a, E) => new AA(A, e, r, i, s, a, E)
    });
    t(this, "_gradients", []);
...
class oA {
  constructor(A, e, r) {
    ...
    }, this.gradientManager = new sA(A, this.fontTextureAtlas), this._renderers = [
      { name: "brightness", renderer: new u(this.p, this.grid, r, H) },
      { name: "accurate", renderer: new P(this.p, this.grid, r, J) },
      { name: "gradient", renderer: new _(this.p, this.grid, r, this.gradientManager, O) },
      { name: "edge", renderer: new I(this.p, this.grid, r, j) },
      { name: "custom", renderer: new l(this.p, this.grid, r, K) }
  ...
  add(A, e, r) {
    switch (e) {
      ...
      case "custom":
        ...
        break;
      default:
        throw new g(`Invalid renderer type: ${e}`);
    }
  }
  /**
...
function gA(o, A, e, r, i, s) {
  ...
  const a = Object.keys(o.gradientParams[A]), E = Object.keys(s).filter((B) => !a.includes(B));
  if (E.length > 0)
    throw new g(
...
class aA {
  ...
  fontSize(A) {
    if (A < c.MIN || A > c.MAX)
      throw new g(`Font size ${A} is out of bounds. It should be between ${c.MIN} and ${c.MAX}.`);
...
n.prototype.registerMethod("init", function() {
  h.instance(this, !1), this._incrementPreload(), h.loadFont(QA);
});
n.prototype.registerMethod("afterSetup", function() {
  BA(this), h.setup(), this.setupAsciify();
});
...
export {
  J as ACCURATE_DEFAULT_OPTIONS,
  H as BRIGHTNESS_DEFAULT_OPTIONS,
  K as CUSTOM_DEFAULT_OPTIONS,
  W as DEFAULT_BACKGROUND_COLOR,
  L as DEFAULT_FONT_SIZE,
  j as EDGE_DEFAULT_OPTIONS,
  c as FONT_SIZE_LIMITS,
  O as GRADIENT_DEFAULT_OPTIONS,
  aA as P5Asciifier,
  ...
  oA as P5AsciifyRendererManager,
