...
class P extends l {
  constructor(e, r, i, s) {
    ...
    this.characterSelectionShader = this.p.createShader(Q, D(this.fontTextureAtlas.fontSize)), this.brightnessSampleShader = this.p.createShader(Q, f(this.grid.cellHeight, this.grid.cellWidth)), this.colorSampleShader = this.p.createShader(Q, m(16, this.grid.cellHeight, this.grid.cellWidth)), this.brightnessSplitShader = this.p.createShader(Q, U), this.brightnessSampleFramebuffer = this.p.createFramebuffer({
      density: 1,
      width: this.grid.cols,
      height: this.grid.rows,
      depthFormat: this.p.UNSIGNED_INT,
...
class p extends l {
  constructor(e, r, i, s) {
    ...
    this.sobelShader = this.p.createShader(Q, z), this.sampleShader = this.p.createShader(Q, I(16, this.grid.cellHeight, this.grid.cellWidth)), this.colorSampleShader = this.p.createShader(Q, M), this.inversionShader = this.p.createShader(Q, Y), this.asciiCharacterShader = this.p.createShader(Q, k), this.sobelFramebuffer = this.p.createFramebuffer({
      depthFormat: this.p.UNSIGNED_INT,
      textureFiltering: this.p.NEAREST
...
class W {
  constructor(A, e) {
    ...
    t(this, "_gradientConstructors", {
      linear: (A, e, r, i, s, a, E) => new N(A, e, r, i, s, a, E),
      spiral: (A, e, r, i, s, a, E) => new X(A, e, r, i, s, a, E),
      radial: (A, e, r, i, s, a, E) => new H(A, e, r, i, s, a, E),
      conical: (A, e, r, i, s, a, E) => new J(A, e, r, i, s, a, E)
    });
    t(this, "_gradients", []);
...
var Z = "precision mediump float;uniform sampler2D u_image;varying vec2 v_texCoord;void main(){vec4 color=texture2D(u_image,v_texCoord);float luminance=0.299*color.r+0.587*color.g+0.114*color.b;color.rgb=vec3(luminance);gl_FragColor=color;}", V = "precision mediump float;uniform sampler2D u_sketchTexture;uniform sampler2D u_previousColorTexture;uniform sampler2D u_sampleTexture;uniform sampler2D u_sampleReferenceTexture;uniform vec2 u_gridCellDimensions;uniform int u_sampleMode;uniform vec4 u_staticColor;void main(){vec2 cellCoord=floor(gl_FragCoord.xy);vec2 cellSizeInTexCoords=vec2(1.0)/u_gridCellDimensions;vec2 cellCenterTexCoord=(cellCoord+vec2(0.5))*cellSizeInTexCoords;bool isMatchingSample=texture2D(u_sampleTexture,cellCenterTexCoord)==texture2D(u_sampleReferenceTexture,cellCenterTexCoord);if(isMatchingSample){gl_FragColor=texture2D(u_previousColorTexture,cellCenterTexCoord);return;}else if(u_sampleMode==0){gl_FragColor=texture2D(u_sketchTexture,cellCenterTexCoord);}else{gl_FragColor=u_staticColor;}}", $ = "precision mediump float;uniform sampler2D u_sampleTexture;uniform sampler2D u_sampleReferenceTexture;uniform sampler2D u_previousInversionTexture;uniform vec2 u_gridCellDimensions;uniform bool u_invert;void main(){vec2 cellCoord=floor(gl_FragCoord.xy);vec2 cellSizeInTexCoords=vec2(1.0)/u_gridCellDimensions;vec2 cellCenterTexCoord=(cellCoord+vec2(0.5))*cellSizeInTexCoords;bool isMatchingSample=texture2D(u_sampleTexture,cellCenterTexCoord)==texture2D(u_sampleReferenceTexture,cellCenterTexCoord);if(!isMatchingSample){if(u_invert){gl_FragColor=vec4(1.0);}else{gl_FragColor=vec4(vec3(0.0),1.0);}return;}else{gl_FragColor=texture2D(u_previousInversionTexture,cellCenterTexCoord);}}", q = "precision mediump float;uniform sampler2D u_prevAsciiCharacterTexture;uniform sampler2D u_prevGradientTexture;uniform sampler2D u_nextGradientTexture;uniform vec2 u_resolution;void main(){vec2 uv=gl_FragCoord.xy/u_resolution;vec4 prevAscii=texture2D(u_prevAsciiCharacterTexture,uv);vec4 prevGradient=texture2D(u_prevGradientTexture,uv);vec4 nextGradient=texture2D(u_nextGradientTexture,uv);bool colorsMatch=prevGradient==nextGradient;gl_FragColor=colorsMatch ? prevAscii : nextGradient;}";
class _ extends l {
  constructor(e, r, i, s) {
    super(e, r, i, s);
    ...
    this.gradientManager = new W(this.p, this.fontTextureAtlas), this.grayscaleShader = this.p.createShader(Q, Z), this.colorSampleShader = this.p.createShader(Q, V), this.inversionShader = this.p.createShader(Q, $), this.asciiCharacterShader = this.p.createShader(Q, q), this.grayscaleFramebuffer = this.p.createFramebuffer({
      density: 1,
      width: this.grid.cols,
      height: this.grid.rows,
      depthFormat: this.p.UNSIGNED_INT,
...
class gA {
  constructor(A, e, r) {
    ...
    this.p = A, this.grid = e, this.fontTextureAtlas = r, this.currentCanvasDimensions = {
      ...
      { name: "brightness", renderer: new u(this.p, this.grid, r, AA) },
      { name: "accurate", renderer: new P(this.p, this.grid, r, eA) },
      { name: "gradient", renderer: new _(this.p, this.grid, r, rA) },
      { name: "edge", renderer: new p(this.p, this.grid, r, tA) },
      { name: "custom", renderer: new l(this.p, this.grid, r, iA) }
  ...
  add(A, e, r) {
    switch (e) {
      ...
      case "accurate":
        ...
        break;
      case "gradient":
        this._renderers.push({ name: A, renderer: new _(this.p, this.grid, this.fontTextureAtlas, r) });
        break;
      case "edge":
        this._renderers.push({ name: A, renderer: new p(this.p, this.grid, this.fontTextureAtlas, r) });
        break;
...
h.prototype.registerMethod("init", function() {
  n.instance(this, !1), this._incrementPreload(), n.loadFont(BA);
});
...
export {
  eA as ACCURATE_DEFAULT_OPTIONS,
  AA as BRIGHTNESS_DEFAULT_OPTIONS,
  iA as CUSTOM_DEFAULT_OPTIONS,
  oA as DEFAULT_BACKGROUND_COLOR,
  sA as DEFAULT_FONT_SIZE,
  tA as EDGE_DEFAULT_OPTIONS,
  c as FONT_SIZE_LIMITS,
  rA as GRADIENT_DEFAULT_OPTIONS,
  aA as P5Asciifier,
  P as P5AsciifyAccurateRenderer,
  u as P5AsciifyBrightnessRenderer,
  w as P5AsciifyColorPalette,
  J as P5AsciifyConicalGradient,
  p as P5AsciifyEdgeRenderer,
  g as P5AsciifyError,
  ...
  F as P5AsciifyGrid,
  N as P5AsciifyLinearGradient,
  H as P5AsciifyRadialGradient,
  l as P5AsciifyRenderer,
  gA as P5AsciifyRendererManager,
  X as P5AsciifySpiralGradient,
  n as p5asciify
};
