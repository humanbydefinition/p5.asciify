...
function M(s, A) {
  if (A != null && A.characterColor && (A.characterColor = s.color(A.characterColor)), A != null && A.backgroundColor && (A.backgroundColor = s.color(A.backgroundColor)), A != null && A.fontSize && (A.fontSize < f.MIN || A.fontSize > f.MAX))
    throw new h(`Font size ${A.fontSize} is out of bounds. It should be between ${f.MIN} and ${f.MAX}.`);
}
class Q {
  constructor(A, e, t, i) {
...
class K extends Q {
  constructor(e, t, i, o) {
    ...
    this._options.characterColor = this.p.color(this._options.characterColor), this._options.backgroundColor = this.p.color(this._options.backgroundColor), this.sobelShader = this.p.createShader(n, J), this.sampleShader = this.p.createShader(n, x(16, this.grid.cellHeight, this.grid.cellWidth)), this.colorSampleShader = this.p.createShader(n, N), this.asciiCharacterShader = this.p.createShader(n, H), this.shader = this.p.createShader(n, C), this.sobelFramebuffer = this.p.createFramebuffer({
      depthFormat: this.p.UNSIGNED_INT,
      textureFiltering: this.p.NEAREST
...
class V extends Q {
  constructor(e, t, i, o, a) {
    ...
    this._options.characterColor = this.p.color(this._options.characterColor), this._options.backgroundColor = this.p.color(this._options.backgroundColor), this.gradientManager = o, this.grayscaleShader = this.p.createShader(n, j), this.colorSampleShader = this.p.createShader(n, L), this.asciiShader = this.p.createShader(n, C), this.grayscaleFramebuffer = this.p.createFramebuffer({
      density: 1,
      width: this.grid.cols,
      height: this.grid.rows,
      depthFormat: this.p.UNSIGNED_INT,
...
class u {
  constructor(A, e, t) {
    ...
    this.p = A, this.asciiFontTextureAtlas = e, this._characters = this.validateCharacters(t), this.characterColors = this.asciiFontTextureAtlas.getCharsetColorArray(this._characters), this.characterColorPalette = new b(this.characterColors), this.characterColorPalette.setup(this.p);
  }
  /**
...
class l {
  ...
  setup(A, e, t) {
    this._shader = e, this._palette = new b(t), this._palette.setup(A);
  }
  setUniforms(A, e, t) {
...
class BA {
  ...
  addGradient(A, e, t, i, o) {
    const a = this.getGradientParams(A, {
      ...
      characters: i,
      ...o
    }), B = this._gradientConstructors[A](
    ...
    return B.registerPaletteChangeCallback(this.handleGradientPaletteChange.bind(this)), this._gradients.push(B), this.p5Instance._setupDone ? B.setup(
      this.p5Instance,
      this.gradientShaders[A],
...
class hA {
  ...
  setup(A, e, t) {
    this.p = A, this.grid = e, this.fontTextureAtlas = t, this.currentCanvasDimensions = {
      ...
      new X(this.p, this.grid, new u(this.p, t, D.characters), { ...Z })
    ];
  }
  /**
...
class QA {
  constructor() {
    ...
    this.borderColor = "#000000", this._fontSize = 16, this.rendererManager = new hA(), this.eventEmitter = new G(), this.postSetupFunction = null, this.postDrawFunction = null;
  }
  /**
...
function EA(s) {
  if (s._renderer.drawingContext instanceof CanvasRenderingContext2D)
    throw new h("WebGL renderer is required for p5.asciify to run.");
  function A(e, t) {
    const [i, o] = [e, t].map((a) => a.split(".").map(Number));
    for (let a = 0; a < Math.max(i.length, o.length); a++) {
      const B = i[a] ?? 0, d = o[a] ?? 0;
      if (B !== d) return B > d ? 1 : -1;
    }
    return 0;
  }
  if (A(s.VERSION, "1.8.0") < 0)
    throw new h("p5.asciify requires p5.js v1.8.0 or higher to run.");
}
function lA(s) {
...
function CA(s, A, e, t, i, o) {
  ...
  const a = (E, m, p, v) => {
    if (typeof E != "number" || E < m || E > p)
      throw new h(
        `Invalid ${v} value '${E}'. Expected a number between ${m} and ${p}.`
      );
  };
  ...
  const B = Object.keys(s.gradientParams[A]), d = Object.keys(o).filter((E) => !B.includes(E));
  if (d.length > 0)
    throw new h(
