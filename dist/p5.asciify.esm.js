...
var mA = (s, e, A) => e in s ? uA(s, e, { enumerable: !0, configurable: !0, writable: !0, value: A }) : s[e] = A;
var i = (s, e, A) => mA(s, typeof e != "symbol" ? e + "" : e, A);
import E from "p5";
class PA {
  /**
...
class F extends sA {
   ...
   * @ignore
   */
  constructor(e, A, r, t = N) {
    super(e, A, { width: A.cols, height: A.rows }, r, { ...N, ...t });
  }
  /**
...
var Q = "precision mediump float;attribute vec3 aPosition;attribute vec2 aTexCoord;varying vec2 v_texCoord;void main(){vec4 positionVec4=vec4(aPosition,1.0);positionVec4.xy=positionVec4.xy*2.0-1.0;gl_Position=positionVec4;v_texCoord=aTexCoord;}", wA = "precision mediump float;uniform sampler2D u_sketchTexture;uniform vec2 u_gridCellDimensions;void main(){vec2 cellCoord=floor(gl_FragCoord.xy);vec2 cellSizeInTexCoords=1.0/u_gridCellDimensions;vec2 cellCenterTexCoord=(cellCoord+vec2(0.5))*cellSizeInTexCoords;vec4 finalColor;finalColor=texture2D(u_sketchTexture,cellCenterTexCoord);gl_FragColor=finalColor;}", bA = "precision mediump float;uniform sampler2D u_colorSampleFramebuffer;uniform sampler2D u_charPaletteTexture;uniform vec2 u_charPaletteSize;uniform vec2 u_textureSize;void main(){vec2 pos=(floor(gl_FragCoord.xy)+0.5)/u_textureSize;vec4 inputColor=texture2D(u_colorSampleFramebuffer,pos);if(inputColor.a==0.0){gl_FragColor=vec4(0.0,0.0,0.0,0.0);return;}float brightness=dot(inputColor.rgb,vec3(0.299,0.587,0.114));float index=clamp(floor(brightness*u_charPaletteSize.x),0.0,u_charPaletteSize.x-1.0);vec3 charColor=texture2D(u_charPaletteTexture,vec2((index+0.5)/u_charPaletteSize.x,0.0)).rgb;gl_FragColor=vec4(charColor,inputColor.a);}";
const H = {
  /** Enable/disable the renderer */
  enabled: !0,
...
class j extends D {
   ...
   * @ignore
   */
  constructor(A, r, t, o = H) {
    super(A, r, t, { ...H, ...o });
    i(this, "colorSampleShader");
    i(this, "asciiCharacterShader");
    i(this, "colorSampleFramebuffer");
    this.colorSampleShader = this._p.createShader(Q, wA), this.asciiCharacterShader = this._p.createShader(Q, bA), this.colorSampleFramebuffer = this._p.createFramebuffer({
      density: 1,
...
class L extends D {
  ...
  constructor(A, r, t, o = J) {
    ...
    this._characterSelectionShader = this._p.createShader(Q, AA(this._fontManager.fontSize)), this._brightnessSampleShader = this._p.createShader(Q, eA(this._grid.cellHeight, this._grid.cellWidth)), this._colorSampleShader = this._p.createShader(Q, rA(16, this._grid.cellHeight, this._grid.cellWidth)), this._brightnessSplitShader = this._p.createShader(Q, xA), this._brightnessSampleFramebuffer = this._p.createFramebuffer({
      density: 1,
      width: this._grid.cols,
      height: this._grid.rows,
      depthFormat: this._p.UNSIGNED_INT,
  ...
  resetShaders() {
    this._characterSelectionShader = this._p.createShader(Q, AA(this._fontManager.fontSize)), this._brightnessSampleShader = this._p.createShader(Q, eA(this._grid.cellHeight, this._grid.cellWidth)), this._colorSampleShader = this._p.createShader(Q, rA(16, this._grid.cellHeight, this._grid.cellWidth));
  }
  render(A) {
...
class W extends D {
  ...
  constructor(A, r, t, o = O) {
    ...
    this.sobelShader = this._p.createShader(Q, MA), this.sampleShader = this._p.createShader(Q, tA(16, this._grid.cellHeight, this._grid.cellWidth)), this.colorSampleShader = this._p.createShader(Q, SA), this.inversionShader = this._p.createShader(Q, vA), this.rotationShader = this._p.createShader(Q, FA), this.asciiCharacterShader = this._p.createShader(Q, yA), this.sobelFramebuffer = this._p.createFramebuffer({
      density: 1,
      depthFormat: this._p.UNSIGNED_INT,
      textureFiltering: this._p.NEAREST
  ...
  resetShaders() {
    this.sampleShader = this._p.createShader(Q, tA(16, this._grid.cellHeight, this._grid.cellWidth));
  }
  /**
...
class oA {
  ...
  constructor(e, A, r) {
    ...
    this._p = e, this._grid = A, this._fontManager = r, this._shader = this._p.createShader(Q, GA), this._resultFramebuffer = this._p.createFramebuffer({
      depthFormat: this._p.UNSIGNED_INT,
      textureFiltering: this._p.NEAREST
    });
  }
...
class gA {
  ...
  constructor(e, A, r) {
    ...
    this._p = e, this._grid = A, this._fontManager = r, this._currentCanvasDimensions = {
      ...
      { name: "custom2D", renderer: new F(this._p, this._grid, this._fontManager) },
      { name: "edge", renderer: new W(this._p, this._grid, this._fontManager) },
      { name: "accurate", renderer: new L(this._p, this._grid, this._fontManager) },
      { name: "brightness", renderer: new j(this._p, this._grid, this._fontManager) }
      ...
      depthFormat: this._p.UNSIGNED_INT,
      textureFiltering: this._p.NEAREST
    }), this._asciiDisplayRenderer2D = new oA(this._p, this._grid, this._fontManager);
  }
  /**
  ...
  render(e) {
    ...
    for (let A = this._renderers.length - 1; A >= 0; A--) {
      ...
      if (r.renderer.options.enabled) {
        r.renderer instanceof D && r.renderer.render(e);
  ...
  resetRendererDimensions() {
    this._primaryColorFramebuffer.resize(this._grid.cols, this._grid.rows), this._secondaryColorFramebuffer.resize(this._grid.cols, this._grid.rows), this._characterFramebuffer.resize(this._grid.cols, this._grid.rows), this._inversionFramebuffer.resize(this._grid.cols, this._grid.rows), this._rotationFramebuffer.resize(this._grid.cols, this._grid.rows), this._renderers.forEach((e) => {
      e.renderer.resizeFramebuffers(), e.renderer instanceof D && e.renderer.resetShaders();
    });
  }
  /**
...
class iA {
  ...
  font(e, A = { updateCharacters: !0 }) {
    this._fontManager.loadFont(e), this._p._setupDone && (this._fontManager.reset(), this._grid.resizeCellPixelDimensions(
      ...
      (r) => {
        r.renderer instanceof D && r.renderer.characters(r.renderer.options.characters);
      }
    ), this._rendererManager.resetRendererDimensions());
  }
  /**
  ...
  saveSVG(e = "") {
    if (!e) {
      const _ = /* @__PURE__ */ new Date(), p = _.toISOString().split("T")[0], a = _.toTimeString().split(" ")[0].replace(/:/g, "-");
      e = `asciify_output_${p}_${a}`;
    }
    const A = this._rendererManager.characterFramebuffer, r = this._rendererManager.primaryColorFramebuffer, t = this._rendererManager.secondaryColorFramebuffer, o = this._rendererManager.inversionFramebuffer, B = this._rendererManager.rotationFramebuffer;
    if (!A)
      throw new g("No renderer available to generate SVG output.");
    A.loadPixels(), r.loadPixels(), t.loadPixels(), o.loadPixels(), B.loadPixels();
    const d = A.pixels, n = r.pixels, C = t.pixels, w = o.pixels, P = B.pixels, y = this._grid.cols, M = this._grid.rows, c = this._grid.cellWidth, b = this._grid.cellHeight, G = this._grid.width, T = this._grid.height, QA = this._fontManager.characterGlyphs, Y = this._fontManager.characters;
    let l = `<?xml version="1.0" encoding="UTF-8" standalone="no"?>
<!DOCTYPE svg PUBLIC "-//W3C//DTD SVG 1.1//EN" "http://www.w3.org/Graphics/SVG/1.1/DTD/svg11.dtd">
<svg width="${G}" height="${T}" viewBox="0 0 ${G} ${T}" 
     xmlns="http://www.w3.org/2000/svg" version="1.1">
<title>ascii art generated via p5.asciify</title>
<desc>ascii art visualization of a p5.js sketch</desc>`;
    const aA = this._rendererManager.asciiDisplayRenderer.backgroundColor, x = this._p.color(aA), EA = `rgba(${x._array[0] * 255},${x._array[1] * 255},${x._array[2] * 255},${x._array[3]})`;
    l += `
<rect width="${G}" height="${T}" fill="${EA}" />`, l += `
<g id="ascii-cells">`;
    let K = 0;
    for (let _ = 0; _ < M; _++)
      for (let p = 0; p < y; p++) {
        const a = K * 4, nA = d[a], lA = d[a + 1];
        let S = nA + (lA << 8);
        S >= Y.length && (S = Y.length - 1), Y[S];
        let u = {
          r: n[a],
          g: n[a + 1],
          b: n[a + 2],
        ...
        const cA = P[a], dA = P[a + 1], X = cA + dA * 256 / 15, k = p * c, U = _ * b;
        if (f.a > 0) {
          const R = `rgba(${f.r},${f.g},${f.b},${f.a / 255})`;
          l += `
  <rect x="${k}" y="${U}" width="${c}" height="${b}" fill="${R}" />`;
        }
        const Z = QA[S], q = `rgba(${u.r},${u.g},${u.b},${u.a / 255})`, z = this._fontSize, CA = k + c / 2, fA = U + b / 2, DA = k + (c - Z.advanceWidth * z / this._fontManager.font.font.unitsPerEm) / 2, _A = U + (b + z * 0.7) / 2, v = Z.getPath(DA, _A, z).toSVG().match(/d="([^"]+)"/);
        v && v[1] && (X > 0 ? (l += `
  <g transform="rotate(${X} ${CA} ${fA})">`, l += `
    <path d="${v[1]}" fill="${q}" />`, l += `
  </g>`) : l += `
  <path d="${v[1]}" fill="${q}" />`), K++;
      }
    ...
    const hA = new Blob([l], { type: "image/svg+xml" }), V = URL.createObjectURL(hA), I = document.createElement("a");
    I.href = V, I.download = `${e}.svg`, document.body.appendChild(I), I.click(), document.body.removeChild(I), URL.revokeObjectURL(V);
  }
  /**
  ...
  _generateAsciiTextOutput() {
    ...
    for (let n = 0; n < t; n++) {
      let C = "";
      for (let w = 0; w < r; w++) {
        const P = d * 4, y = A[P], M = A[P + 1];
        let c = y + (M << 8);
        c >= o.length && (c = o.length - 1), C += o[c], d++;
      }
      B.push(C);
    }
    return B;
...
const BA = (s, e) => {
  ...
  __proto__: null,
  ACCURATE_DEFAULT_OPTIONS: J,
  AbstractFeatureRenderer2D: D,
  BRIGHTNESS_DEFAULT_OPTIONS: H,
  EDGE_DEFAULT_OPTIONS: O,
  P5AsciifyAccurateRenderer: L,
  P5AsciifyBrightnessRenderer: j,
  P5AsciifyEdgeRenderer: W
}, Symbol.toStringTag, { value: "Module" })), UA = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  CUSTOM_DEFAULT_OPTIONS_2D: N,
  P5AsciifyRenderer2D: F,
  feature: kA
}, Symbol.toStringTag, { value: "Module" })), KA = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  __proto__: null,
  "2d": UA,
  P5AsciifyDisplayRenderer: oA,
  P5AsciifyRenderer: sA,
  P5AsciifyRendererManager: gA,
  RENDERER_TYPES: $
...
class m {
  constructor(e, A, r, t) {
    ...
    i(this, "y");
    i(this, "z");
    this.w = e, this.x = A, this.y = r, this.z = t;
  }
  static angleAxis(e, A) {
    const r = e / 2, t = Math.sin(r);
    return new m(Math.cos(r), A[0] * t, A[1] * t, A[2] * t).normalize();
  }
  multiply(e) {
    const A = this.w * e.w - this.x * e.x - this.y * e.y - this.z * e.z, r = this.w * e.x + this.x * e.w + this.y * e.z - this.z * e.y, t = this.w * e.y - this.x * e.z + this.y * e.w + this.z * e.x, o = this.w * e.z + this.x * e.y - this.y * e.x + this.z * e.w;
    return new m(A, r, t, o);
  }
  normalize() {
    const e = Math.hypot(this.w, this.x, this.y, this.z);
    return e === 0 ? new m(1, 0, 0, 0) : new m(this.w / e, this.x / e, this.y / e, this.z / e);
  }
}
const VA = /* @__PURE__ */ Object.freeze(/* @__PURE__ */ Object.defineProperty({
  ...
  Quaternion: m,
  compareVersions: BA
}, Symbol.toStringTag, { value: "Module" })), h = new YA();
typeof window < "u" && (window.p5asciify = h);
const zA = (s) => {
  h.hooksEnabled && h.init(s);
};
E.prototype.registerMethod("init", function() {
  zA(this);
});
const RA = (s) => {
  h.hooksEnabled && setTimeout(() => {
