...
var R = (o, A, e) => A in o ? z(o, A, { enumerable: !0, configurable: !0, writable: !0, value: e }) : o[A] = e;
var i = (o, A, e) => R(o, typeof A != "symbol" ? A + "" : A, e);
import n from "p5";
...
class N {
  ...
  constructor(A, e, r) {
    ...
    const t = Object.values(this.font.font.glyphs.glyphs);
    this._characters = t.filter((s) => s.unicode !== void 0).map((s) => String.fromCharCode(s.unicode)), this._characterGlyphs = this._loadCharacterGlyphs(), this._maxGlyphDimensions = this._getMaxGlyphDimensions(this._fontSize), this._createTexture(this._fontSize);
  }
  /**
  ...
  drawCharacters(A) {
    ...
    for (let e = 0; e < this._characterGlyphs.length; e++) {
      const r = e % this._charsetCols, t = Math.floor(e / this._charsetCols), s = this._maxGlyphDimensions.width * r - this._maxGlyphDimensions.width * this._charsetCols / 2, a = this._maxGlyphDimensions.height * t - this._maxGlyphDimensions.height * this._charsetRows / 2;
...
class F {
  ...
  updateFramebuffer() {
    ...
    for (let r = 0; r < A; r++) {
      const t = r < this._colors.length ? this.p.color(this._colors[r]) : this.p.color(0, 0, 0, 0), s = 4 * r;
      this.framebuffer.pixels[s] = this.p.red(t), this.framebuffer.pixels[s + 1] = this.p.green(t), this.framebuffer.pixels[s + 2] = this.p.blue(t), this.framebuffer.pixels[s + 3] = this.p.alpha(t);
    }
...
class l {
  constructor(A, e, r, t = C) {
    ...
    i(this, "_shader");
    this.p = A, this.grid = e, this.fontTextureAtlas = r, this._options = t, this._options = { ...C, ...t }, this.characterColorPalette = new F(this.p, this.fontTextureAtlas.getCharsetColorArray(this._options.characters)), this._primaryColorFramebuffer = this.p.createFramebuffer({
      density: 1,
      antialias: !1,
      ...
      depthFormat: this.p.UNSIGNED_INT,
      textureFiltering: this.p.NEAREST
    }), this._shader = this.p.createShader(Q, H), this.update(this._options);
  }
  /**
   * Resizes all framebuffers based on the grid dimensions.
   */
  resizeFramebuffers() {
    this._primaryColorFramebuffer.resize(this.grid.cols, this.grid.rows), this._secondaryColorFramebuffer.resize(this.grid.cols, this.grid.rows), this._inversionFramebuffer.resize(this.grid.cols, this.grid.rows), this._characterFramebuffer.resize(this.grid.cols, this.grid.rows);
  }
  /**
  ...
  characters(A = "") {
    ...
    this.fontTextureAtlas.validateCharacters(A), this.characterColorPalette.setColors(this.fontTextureAtlas.getCharsetColorArray(A)), this.resetShaders(), this._options.characters = A;
  }
  /**
...
class P extends l {
  constructor(e, r, t, s = u) {
    const a = { ...u, ...s };
    super(e, r, t, a);
    i(this, "colorSampleShader");
    i(this, "asciiCharacterShader");
    i(this, "colorSampleFramebuffer");
    this.colorSampleShader = this.p.createShader(Q, J), this.asciiCharacterShader = this.p.createShader(Q, O), this.colorSampleFramebuffer = this.p.createFramebuffer({
      density: 1,
      width: this.grid.cols,
      height: this.grid.rows,
      depthFormat: this.p.UNSIGNED_INT,
  ...
  render(e, r) {
    this.colorSampleFramebuffer.begin(), this.p.clear(), this.p.shader(this.colorSampleShader), this.colorSampleShader.setUniform("u_sketchTexture", e), this.colorSampleShader.setUniform("u_gridCellDimensions", [this.grid.cols, this.grid.rows]), this.p.rect(0, 0, this.p.width, this.p.height), this.colorSampleFramebuffer.end(), this._primaryColorFramebuffer.begin(), this._options.characterColorMode === 1 ? this.p.background(this._options.characterColor) : (this.p.clear(), this.p.image(this.colorSampleFramebuffer, -this.grid.cols / 2, -this.grid.rows / 2, this.grid.cols, this.grid.rows)), this._primaryColorFramebuffer.end(), this._secondaryColorFramebuffer.begin(), this._options.backgroundColorMode === 1 ? this.p.background(this._options.backgroundColor) : (this.p.clear(), this.p.image(this.colorSampleFramebuffer, -this.grid.cols / 2, -this.grid.rows / 2, this.grid.cols, this.grid.rows)), this._secondaryColorFramebuffer.end(), this._inversionFramebuffer.begin(), this.p.clear(), this._options.invertMode ? this.p.background(255) : this.p.background(0), this._inversionFramebuffer.end(), this._characterFramebuffer.begin(), this.p.clear(), this.p.shader(this.asciiCharacterShader), this.asciiCharacterShader.setUniform("u_textureSize", [this.grid.cols, this.grid.rows]), this.asciiCharacterShader.setUniform("u_colorSampleFramebuffer", this.colorSampleFramebuffer), this.asciiCharacterShader.setUniform("u_charPaletteTexture", this.characterColorPalette.framebuffer), this.asciiCharacterShader.setUniform("u_charPaletteSize", [this.characterColorPalette.colors.length, 1]), this.p.rect(0, 0, this.p.width, this.p.height), this._characterFramebuffer.end(), super.render(e, r);
  }
}
const w = (o) => `
...
class I extends l {
  constructor(e, r, t, s = f) {
    ...
    this.characterSelectionShader = this.p.createShader(Q, w(this.fontTextureAtlas.fontSize)), this.brightnessSampleShader = this.p.createShader(Q, x(this.grid.cellHeight, this.grid.cellWidth)), this.colorSampleShader = this.p.createShader(Q, b(16, this.grid.cellHeight, this.grid.cellWidth)), this.brightnessSplitShader = this.p.createShader(Q, j), this.brightnessSampleFramebuffer = this.p.createFramebuffer({
      density: 1,
      width: this.grid.cols,
      height: this.grid.rows,
      depthFormat: this.p.UNSIGNED_INT,
...
class _ extends l {
  constructor(e, r, t, s = D) {
    ...
    this.sobelShader = this.p.createShader(Q, V), this.sampleShader = this.p.createShader(Q, v(16, this.grid.cellHeight, this.grid.cellWidth)), this.colorSampleShader = this.p.createShader(Q, K), this.inversionShader = this.p.createShader(Q, W), this.asciiCharacterShader = this.p.createShader(Q, L), this.sobelFramebuffer = this.p.createFramebuffer({
      depthFormat: this.p.UNSIGNED_INT,
      textureFiltering: this.p.NEAREST
  ...
  resetShaders() {
    this.sampleShader = this.p.createShader(Q, v(16, this.grid.cellHeight, this.grid.cellWidth));
  }
  /**
...
const d = (o, A, e, r) => {
  ...
  for (let t = 0; t < Math.max(e.length, r.length); t++) {
    const s = e[t] ?? 0, a = r[t] ?? 0;
    if (s !== a) return s > a ? 1 : -1;
  }
  return 0;
};
class c {
  constructor(A, e, r, t, s, a) {
    i(this, "_brightnessStart");
    i(this, "_brightnessEnd");
    i(this, "enabled");
    i(this, "_onPaletteChangeCallback");
    i(this, "_palette");
    this.p = A, this._fontTextureAtlas = e, this._shader = r, this._characters = t, this._palette = new F(this.p, this._fontTextureAtlas.getCharsetColorArray(this._characters)), this._brightnessStart = Math.floor(s / 255 * 100) / 100, this._brightnessEnd = Math.ceil(a / 255 * 100) / 100, this.enabled = !0;
  }
  /**
  ...
  characters(A) {
    if (typeof A != "string")
      throw new g("Characters must be a string.");
    this._fontTextureAtlas.validateCharacters(A), this.palette.setColors(this._fontTextureAtlas.getCharsetColorArray(A));
  }
  // Getters
  get shader() {
    return this._shader;
...
class T extends c {
  constructor(e, r, t, s, a, E, B) {
    super(e, r, t, s, a, E);
    i(this, "direction");
    i(this, "angle");
    i(this, "speed");
    i(this, "zigzag");
    this.p = e, this._fontTextureAtlas = r, this._shader = t, this._characters = s, this.direction = B.direction, this.angle = B.angle, this.speed = B.speed, this.zigzag = B.zigzag;
  }
  setUniforms(e, r) {
...
class y extends c {
  constructor(e, r, t, s, a, E, B) {
    ...
    i(this, "speed");
    i(this, "density");
    this.p = e, this._fontTextureAtlas = r, this._shader = t, this._characters = s, this.direction = B.direction, this.centerX = B.centerX, this.centerY = B.centerY, this.speed = B.speed, this.density = B.density;
  }
  setUniforms(e, r) {
...
class G extends c {
  constructor(e, r, t, s, a, E, B) {
    ...
    i(this, "centerY");
    i(this, "radius");
    this.p = e, this._fontTextureAtlas = r, this._shader = t, this._characters = s, this.direction = B.direction, this.centerX = B.centerX, this.centerY = B.centerY, this.radius = B.radius;
  }
  setUniforms(e, r) {
...
class U extends c {
  constructor(e, r, t, s, a, E, B) {
    ...
    i(this, "centerY");
    i(this, "speed");
    this.p = e, this._fontTextureAtlas = r, this._shader = t, this._characters = s, this.centerX = B.centerX, this.centerY = B.centerY, this.speed = B.speed;
  }
  setUniforms(e, r) {
...
class M {
  constructor(A, e) {
    ...
    i(this, "_gradientConstructors", {
      linear: (A, e, r, t, s, a, E) => new T(A, e, r, t, s, a, E),
      spiral: (A, e, r, t, s, a, E) => new y(A, e, r, t, s, a, E),
      radial: (A, e, r, t, s, a, E) => new G(A, e, r, t, s, a, E),
      conical: (A, e, r, t, s, a, E) => new U(A, e, r, t, s, a, E)
    });
    i(this, "_gradients", []);
    ...
    for (const r of Object.keys(this._gradientShaderSources)) {
      ...
      this._gradientShaders[r] = this._p.createShader(Q, t);
    }
  }
  /**
  ...
  add(A, e, r, t, s) {
    if (typeof A != "string")
      throw new g("Gradient name must be a string");
    if (!this.gradientConstructors[A])
      throw new g(
        `Gradient '${A}' does not exist! Available gradients: ${Object.keys(this.gradientConstructors).join(", ")}`
      );
    if (typeof r != "number")
      throw new g("Brightness start value must be a number");
    if (typeof t != "number")
      throw new g("Brightness end value must be a number");
    if (d(r, 0, 255, "brightness start"), d(t, 0, 255, "brightness end"), typeof e != "string")
      throw new g("Characters must be a string");
    if (e.length === 0)
      throw new g("Characters string cannot be empty");
    if (!s || typeof s != "object" || Array.isArray(s))
      throw new g("User parameters must be an object");
    const a = Object.keys(this.gradientParams[A]), E = Object.keys(s).filter((k) => !a.includes(k));
    if (E.length > 0)
      throw new g(
...
Valid parameters are: ${a.join(", ")}`
    ...
    return this._gradients.push(B), B;
  }
  /**
...
var rA = "precision mediump float;uniform sampler2D u_image;varying vec2 v_texCoord;void main(){vec4 color=texture2D(u_image,v_texCoord);float luminance=0.299*color.r+0.587*color.g+0.114*color.b;color.rgb=vec3(luminance);gl_FragColor=color;}", tA = "precision mediump float;uniform sampler2D u_sketchTexture;uniform sampler2D u_previousColorTexture;uniform sampler2D u_sampleTexture;uniform sampler2D u_sampleReferenceTexture;uniform vec2 u_gridCellDimensions;uniform int u_sampleMode;uniform vec4 u_staticColor;void main(){vec2 cellCoord=floor(gl_FragCoord.xy);vec2 cellSizeInTexCoords=vec2(1.0)/u_gridCellDimensions;vec2 cellCenterTexCoord=(cellCoord+vec2(0.5))*cellSizeInTexCoords;bool isMatchingSample=texture2D(u_sampleTexture,cellCenterTexCoord)==texture2D(u_sampleReferenceTexture,cellCenterTexCoord);if(isMatchingSample){gl_FragColor=texture2D(u_previousColorTexture,cellCenterTexCoord);return;}else if(u_sampleMode==0){gl_FragColor=texture2D(u_sketchTexture,cellCenterTexCoord);}else{gl_FragColor=u_staticColor;}}", iA = "precision mediump float;uniform sampler2D u_sampleTexture;uniform sampler2D u_sampleReferenceTexture;uniform sampler2D u_previousInversionTexture;uniform vec2 u_gridCellDimensions;uniform bool u_invert;void main(){vec2 cellCoord=floor(gl_FragCoord.xy);vec2 cellSizeInTexCoords=vec2(1.0)/u_gridCellDimensions;vec2 cellCenterTexCoord=(cellCoord+vec2(0.5))*cellSizeInTexCoords;bool isMatchingSample=texture2D(u_sampleTexture,cellCenterTexCoord)==texture2D(u_sampleReferenceTexture,cellCenterTexCoord);if(!isMatchingSample){if(u_invert){gl_FragColor=vec4(1.0);}else{gl_FragColor=vec4(vec3(0.0),1.0);}return;}else{gl_FragColor=texture2D(u_previousInversionTexture,cellCenterTexCoord);}}", sA = "precision mediump float;uniform sampler2D u_prevAsciiCharacterTexture;uniform sampler2D u_prevGradientTexture;uniform sampler2D u_nextGradientTexture;uniform vec2 u_resolution;void main(){vec2 uv=gl_FragCoord.xy/u_resolution;vec4 prevAscii=texture2D(u_prevAsciiCharacterTexture,uv);vec4 prevGradient=texture2D(u_prevGradientTexture,uv);vec4 nextGradient=texture2D(u_nextGradientTexture,uv);bool colorsMatch=prevGradient==nextGradient;gl_FragColor=colorsMatch ? prevAscii : nextGradient;}";
const m = {
  /** Enable/disable the renderer */
  enabled: !1,
  /** Color of the ASCII characters. Only used when `characterColorMode` is set to `fixed` */
...
class p extends l {
  constructor(e, r, t, s = m) {
    ...
    this.gradientManager = new M(this.p, this.fontTextureAtlas), this.grayscaleShader = this.p.createShader(Q, rA), this.colorSampleShader = this.p.createShader(Q, tA), this.inversionShader = this.p.createShader(Q, iA), this.asciiCharacterShader = this.p.createShader(Q, sA), this.grayscaleFramebuffer = this.p.createFramebuffer({
      density: 1,
      width: this.grid.cols,
      height: this.grid.rows,
      depthFormat: this.p.UNSIGNED_INT,
  ...
  add(e, r, t, s, a = {}) {
    return this.gradientManager.add(e, s, r, t, a);
  }
  remove(e) {
  ...
  render(e, r) {
    ...
    for (const t of this.gradientManager.gradients)
      t.enabled && ([this.prevAsciiGradientFramebuffer, this.nextAsciiGradientFramebuffer] = [this.nextAsciiGradientFramebuffer, this.prevAsciiGradientFramebuffer], this.nextAsciiGradientFramebuffer.begin(), this.p.clear(), this.p.shader(t.shader), t.setUniforms(this.prevAsciiGradientFramebuffer, this.grayscaleFramebuffer), this.p.rect(0, 0, this.grid.cols, this.grid.rows), this.nextAsciiGradientFramebuffer.end());
    this._characterFramebuffer.begin(), this.p.clear(), this.p.shader(this.asciiCharacterShader), r !== this && this.asciiCharacterShader.setUniform("u_prevAsciiCharacterTexture", r.characterFramebuffer), this.asciiCharacterShader.setUniform("u_prevGradientTexture", this.grayscaleFramebuffer), this.asciiCharacterShader.setUniform("u_nextGradientTexture", this.nextAsciiGradientFramebuffer), this.asciiCharacterShader.setUniform("u_resolution", [this.grid.cols, this.grid.rows]), this.p.rect(0, 0, this.grid.cols, this.grid.rows), this._characterFramebuffer.end(), this._primaryColorFramebuffer.begin(), this.p.clear(), this.p.shader(this.colorSampleShader), this.colorSampleShader.setUniform("u_sketchTexture", e), r !== this && this.colorSampleShader.setUniform("u_previousColorTexture", r.primaryColorFramebuffer), this.colorSampleShader.setUniform("u_sampleTexture", this.nextAsciiGradientFramebuffer), this.colorSampleShader.setUniform("u_sampleReferenceTexture", this.grayscaleFramebuffer), this.colorSampleShader.setUniform("u_gridCellDimensions", [this.grid.cols, this.grid.rows]), this.colorSampleShader.setUniform("u_sampleMode", this._options.characterColorMode), this.colorSampleShader.setUniform("u_staticColor", this._options.characterColor._array), this.p.rect(0, 0, this.p.width, this.p.height), this._primaryColorFramebuffer.end(), this._secondaryColorFramebuffer.begin(), this.p.clear(), this.p.shader(this.colorSampleShader), this.colorSampleShader.setUniform("u_sketchTexture", e), r !== this && this.colorSampleShader.setUniform("u_previousColorTexture", r.secondaryColorFramebuffer), this.colorSampleShader.setUniform("u_sampleTexture", this.nextAsciiGradientFramebuffer), this.colorSampleShader.setUniform("u_sampleReferenceTexture", this.grayscaleFramebuffer), this.colorSampleShader.setUniform("u_gridCellDimensions", [this.grid.cols, this.grid.rows]), this.colorSampleShader.setUniform("u_sampleMode", this._options.backgroundColorMode), this.colorSampleShader.setUniform("u_staticColor", this._options.backgroundColor._array), this.p.rect(0, 0, this.p.width, this.p.height), this._secondaryColorFramebuffer.end(), this._inversionFramebuffer.begin(), this.p.clear(), this.p.shader(this.inversionShader), this.inversionShader.setUniform("u_invert", this._options.invertMode), this.inversionShader.setUniform("u_gridCellDimensions", [this.grid.cols, this.grid.rows]), this.inversionShader.setUniform("u_sampleTexture", this.nextAsciiGradientFramebuffer), this.inversionShader.setUniform("u_sampleReferenceTexture", this.grayscaleFramebuffer), r !== this && this.inversionShader.setUniform("u_previousInversionTexture", r.inversionFramebuffer), this.p.rect(0, 0, this.p.width, this.p.height), this._inversionFramebuffer.end(), super.render(e, r);
  }
}
const S = {
  brightness: P,
...
class Y {
  constructor(A, e, r) {
    ...
    this.p = A, this.grid = e, this.fontTextureAtlas = r, this.currentCanvasDimensions = {
      ...
      { name: "custom", renderer: new l(this.p, this.grid, r) },
      { name: "edge", renderer: new _(this.p, this.grid, r) },
      { name: "gradient", renderer: new p(this.p, this.grid, r) },
      { name: "accurate", renderer: new I(this.p, this.grid, r) },
      { name: "brightness", renderer: new P(this.p, this.grid, r) }
  ...
  render(A) {
    ...
    for (let r = this._renderers.length - 1; r >= 0; r--) {
      const t = this._renderers[r];
      t.renderer.options.enabled && (t.renderer.render(A, e), t.renderer.outputFramebuffer, e = t.renderer, this.lastRenderer = t.renderer);
    }
  ...
  add(A, e, r) {
    ...
    if (!t)
      throw new g(
        `Invalid renderer type: ${e}. Valid types are: ${Object.keys(S).join(", ")}`
      );
    const s = new t(this.p, this.grid, this.fontTextureAtlas, r);
   ...
   */
  swap(A, e) {
    const r = this.getRendererIndex(A), t = this.getRendererIndex(e);
    if (r === -1 || t === -1)
...
class oA {
   ...
   */
  asciify() {
    this._p.clear(), this._rendererManager.renderers.length > 0 ? (this._rendererManager.render(this._sketchFramebuffer), this._p.background(this._backgroundColor), this._p.image(this._rendererManager.lastRenderer.outputFramebuffer, -this._p.width / 2, -this._p.height / 2)) : this._p.image(this._sketchFramebuffer, -this._p.width / 2, -this._p.height / 2);
  }
  /**
  ...
  fontSize(A) {
    if (A < 1 || A > 128)
      throw new g(`Font size ${A} is out of bounds. It should be between 1 and 128.`);
  ...
  loadFont(A) {
    ...
    ) : this._font = A, this._p._setupDone && (this._fontTextureAtlas.setFontObject(A), this._rendererManager.renderers.forEach((e) => e.renderer.characters(e.renderer.options.characters)), this._grid.resizeCellPixelDimensions(
      this._fontTextureAtlas.maxGlyphDimensions.width,
      this._fontTextureAtlas.maxGlyphDimensions.height
    ));
  }
...
export {
  oA as P5Asciifier,
  F as P5AsciifyColorPalette,
  g as P5AsciifyError,
  N as P5AsciifyFontTextureAtlas,
  X as P5AsciifyGrid,
  EA as gradients,
  h as p5asciify,
  QA as renderers
