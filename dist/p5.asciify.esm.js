...
var i = (r, A, e) => v(r, typeof A != "symbol" ? A + "" : A, e);
import B from "p5";
...
class F {
  ...
  drawCharacters(A) {
    ...
    for (let e = 0; e < this._characterGlyphs.length; e++) {
      const t = e % this._charsetCols, s = Math.floor(e / this._charsetCols), o = this._maxGlyphDimensions.width * t - this._maxGlyphDimensions.width * this._charsetCols / 2, Q = this._maxGlyphDimensions.height * s - this._maxGlyphDimensions.height * this._charsetRows / 2;
      this.p.text(String.fromCharCode(this._characterGlyphs[e].unicode), o, Q);
    }
  }
  /**
...
function T(r) {
  ...
  if (r != null && r.characterColor && !(typeof r.characterColor == "string" || r.characterColor instanceof B.Color || Array.isArray(r.characterColor) && r.characterColor.length === 3 && r.characterColor.every((A) => typeof A == "number")))
    throw new a("`characterColor` must be a string, p5.Color, or array of 3 numbers.");
  if (r != null && r.backgroundColor && !(typeof r.backgroundColor == "string" || r.backgroundColor instanceof B.Color || Array.isArray(r.backgroundColor) && r.backgroundColor.length === 3 && r.backgroundColor.every((A) => typeof A == "number")))
...
class k extends n {
  constructor(e, t, s, o) {
    ...
    this.characterSelectionShader = this.p.createShader(g, D(this.characterSet.asciiFontTextureAtlas.fontSize)), this.brightnessSampleShader = this.p.createShader(g, f(this.grid.cellHeight, this.grid.cellWidth)), this.colorSampleShader = this.p.createShader(g, m(16, this.grid.cellHeight, this.grid.cellWidth)), this.brightnessSplitShader = this.p.createShader(g, z), this.brightnessSampleFramebuffer = this.p.createFramebuffer({
      density: 1,
      width: this.grid.cols,
      height: this.grid.rows,
      depthFormat: this.p.UNSIGNED_INT,
  ...
  resetShaders() {
    this.characterSelectionShader = this.p.createShader(g, D(this.characterSet.asciiFontTextureAtlas.fontSize)), this.brightnessSampleShader = this.p.createShader(g, f(this.grid.cellHeight, this.grid.cellWidth)), this.colorSampleShader = this.p.createShader(g, m(16, this.grid.cellHeight, this.grid.cellWidth));
  }
  render(e, t) {
...
class J extends n {
  constructor(e, t, s, o) {
    ...
    this.sobelShader = this.p.createShader(g, O), this.sampleShader = this.p.createShader(g, P(16, this.grid.cellHeight, this.grid.cellWidth)), this.colorSampleShader = this.p.createShader(g, R), this.asciiCharacterShader = this.p.createShader(g, N), this.sobelFramebuffer = this.p.createFramebuffer({
      depthFormat: this.p.UNSIGNED_INT,
      textureFiltering: this.p.NEAREST
  ...
  updateOptions(e) {
    if (super.updateOptions(e), e != null && e.characters && e.characters.length !== _)
      throw new a(`For EdgeAsciiRenderer, 'characters' must be a string of length ${_}.`);
  }
  render(e, t) {
...
const d = (r, A, e, t) => {
  ...
  const [e, t] = [r, A].map((s) => s.split(".").map(Number));
  for (let s = 0; s < Math.max(e.length, t.length); s++) {
    const o = e[s] ?? 0, Q = t[s] ?? 0;
    if (o !== Q) return o > Q ? 1 : -1;
  }
  return 0;
};
class l {
...
class aA {
  constructor(A, e) {
    ...
    i(this, "_gradientConstructors", {
      linear: (A, e, t, s) => new Z(A, e, t, s),
      spiral: (A, e, t, s) => new $(A, e, t, s),
      radial: (A, e, t, s) => new q(A, e, t, s),
      conical: (A, e, t, s) => new AA(A, e, t, s),
      noise: (A, e, t, s) => new eA(A, e, t, s)
    });
    i(this, "_gradients", []);
    this.p = A, this.fontTextureAtlas = e, this.setupShaders();
  }
  /**
  ...
  addGradient(A, e, t, s, o) {
    const Q = this._gradientConstructors[A](
      e,
      t,
      s,
      { ...this._gradientParams[A], ...o }
    );
    return this._gradients.push(Q), Q.setup(
      this.p,
...
class gA {
  constructor(A, e, t) {
    ...
    }, this.gradientManager = new aA(A, this.fontTextureAtlas), this.gradientCharacterSet = new h(
      this.p,
      t,
      c.characters
    ), this._renderers = [
      new Y(this.p, this.grid, new h(this.p, t, c.characters), { ...c }),
      new k(this.p, this.grid, new h(this.p, t, I.characters), { ...I }),
      new V(this.p, this.grid, this.gradientCharacterSet, this.gradientManager, { ...X }),
      new J(this.p, this.grid, new h(this.p, t, p.characters), { ...p }),
      new n(this.p, this.grid, new h(this.p, t, c.characters), { ...H })
    ];
  }
  /**
...
function lA(r, A, e, t, s, o) {
  ...
  const Q = Object.keys(r.gradientParams[A]), u = Object.keys(o).filter((b) => !Q.includes(b));
  if (u.length > 0)
    throw new a(
      `Invalid parameter(s) for gradient '${A}': ${u.join(", ")}
Valid parameters are: ${Q.join(", ")}`
    );
}
function cA(r) {
  B.prototype.addAsciiGradient = function(A, e, t, s, o = {}) {
    return lA(
