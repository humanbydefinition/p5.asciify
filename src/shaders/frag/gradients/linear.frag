...
void main() {
    vec4 texColor = texture(textureID, v_texCoord);
    vec4 originalTexColor = texture(originalTextureID, v_texCoord);
    ...
    if(texColor.r >= u_brightnessRange[0] && texColor.r <= u_brightnessRange[1] && texColor == originalTexColor) {
        ...
        index = floor(index);
        float texelPosition = (index + 0.5f) / gradientTextureDimensions.x;

        // Calculate the correct row position for the gradient texture
        float rowPosition = float(gradientTextureRow) + 0.5f; // rowIndex is an integer starting from 0
        float rowTexCoord = rowPosition / gradientTextureDimensions.y;

        vec4 gradientColor = texture(gradientTexture, vec2(texelPosition, rowTexCoord));
        fragColor = vec4(gradientColor.rgb, texColor.a);
    } else {
        fragColor = texColor;
    }
}