...
void main() {
    ...
    if(texColor.r >= u_brightnessRange[0] && texColor.r <= u_brightnessRange[1] && texColor == originalTexColor) {
        ...
        float animatedIndex = mod(index + float(frameCount) * 0.1 * float(-u_gradientDirection), gradientTextureLength);
        ...
        vec4 gradientColor = texture(gradientTexture, vec2(normalizedIndex, rowTexCoord));
