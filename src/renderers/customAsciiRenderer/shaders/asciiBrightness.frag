...
void main() {
    ...
    float brightness = dot(encodedIndexVec.rgb, vec3(0.299, 0.587, 0.114));
    ...
    if(u_backgroundColorMode == 0) {
        gl_FragColor = mix(vec4(secondaryColor.rgb, 1.0), finalColor, charColor.a);
    } else {
    ...
    if(outsideBounds) {
        gl_FragColor = (u_backgroundColorMode == 0) ? (u_invertMode == 1 ? (u_characterColorMode == 0 ? vec4(secondaryColor.rgb, 1.0) : vec4(u_characterColor, 1.0)) : vec4(secondaryColor.rgb, 1.0)) : (u_invertMode == 1 ? (u_characterColorMode == 0 ? vec4(secondaryColor.rgb, 1.0) : vec4(u_characterColor, 1.0)) : vec4(u_backgroundColor, 1.0));
    }
}